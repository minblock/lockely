...
*Setup instructions for a Gitian build of Lockely Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lockely
...
- [Building Lockely Core](#building-lockely-core)
...
Clone the git repositories for lockely and Gitian.
...
in the lockely repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Lockely Core
----------------

To build Lockely Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lockely repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lockely/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lockely-project/lockely
...
For example:
```bash
URL=https://github.com/thrasher-/lockely.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lockely=${COMMIT} --url lockely=${URL} ../lockely/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lockely=${COMMIT} --url lockely=${URL} ../lockely/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lockely=${COMMIT} --url lockely=${URL} ../lockely/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lockely git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lockely/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lockely
SIGPATH=/some/root/path/lockely-detached-sigs

./bin/gbuild --url lockely=${BTCPATH},signature=${SIGPATH} ../lockely/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lockely-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lockely-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lockely-osx-build.assert
```
...
[lockely-project/gitian.sigs.ltc](https://github.com/lockely-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
