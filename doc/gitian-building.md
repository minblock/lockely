...
*Setup instructions for a Gitian build of Lockelycoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lockelycoin
...
- [Building Lockelycoin](#building-lockelycoin)
...
Clone the git repositories for lockelycoin and Gitian and then checkout the lockelycoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the lockelycoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for Gitian to work offline.

Building Lockelycoin
----------------

To build Lockelycoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the lockelycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lockelycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/lockelycoin-project/lockelycoin
...
For example:
```bash
URL=https://github.com/user/lockelycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lockelycoin=${COMMIT} --url lockelycoin=${URL} ../lockelycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lockelycoin=${COMMIT} --url lockelycoin=${URL} ../lockelycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lockelycoin=${COMMIT} --url lockelycoin=${URL} ../lockelycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lockelycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lockelycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/lockelycoin-build.assert
```
...
[lockelycoin/gitian.sigs](https://github.com/lockelycoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
